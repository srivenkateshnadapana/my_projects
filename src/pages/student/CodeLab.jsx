...
import { Play, Code2, RefreshCw, Terminal, File, FileText, Plus, Download, X, Settings2, FolderTree, MoreVertical, Edit2, Copy, Trash2, Check } from 'lucide-react';
...
function CodeLabContent() {
  ...
  const [editorTheme, setEditorTheme] = useState(localStorage.getItem('codelab-theme') || 'vs-dark');
  ...
  const handleDownloadWorkspace = async () => {
    ...
    saveAs(blob, "adhoc-workspace.zip");
  ...
  const runCode = () => {
    ...
    setOutput([`Execution for ${activeFile.language} is mocked in browser.`, "HTML/JS files run natively. Please run index.html to test web projects."]);
    setIsRunning(false);
  };
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-6 gap-4">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <h1 className="text-3xl font-headline font-bold text-primary italic">CodeLab Workspace</h1>
            </div>
            <p className="text-on-surface-variant text-sm">HTML Bundler & Multi-file Execution Environment</p>
          </div>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <DropdownMenu.Root>
              ...
              <DropdownMenu.Portal>
                <DropdownMenu.Content className="min-w-[180px] bg-surface-container-high border border-surface-dim rounded-xl shadow-2xl p-2 z-50 animate-in fade-in zoom-in duration-200" sideOffset={5}>
                  <div className="px-2 py-1.5 text-xs font-bold text-on-surface-variant uppercase tracking-widest mb-1">Editor Theme</div>
                  {Object.entries(THEMES).map(([key, label]) => (
                    ...
                    >
                      {label}
                      {editorTheme === key && <Check className="w-4 h-4 text-primary" />}
                    </DropdownMenu.Item>
                  ))}
            ...
            <button 
              ...
              className="px-5 py-2.5 bg-surface-container-high text-primary rounded-xl font-bold flex items-center gap-2 hover:bg-surface-dim transition-all shadow-md"
            >
                  ...
                  {editingFile === file.name ? (
                    ...
                    >
                      <div className="flex items-center gap-2 truncate">
                        <FileText className={`w-4 h-4 shrink-0 ${activeFilename === file.name ? 'text-primary' : 'opacity-60'}`} />
                      ...
                      <DropdownMenu.Root>
                        ...
                        <DropdownMenu.Portal>
                          <DropdownMenu.Content className="min-w-[160px] bg-surface-container-high border border-surface-dim rounded-xl shadow-xl p-1 z-50 animate-in fade-in" sideOffset={5} align="start">
                            <DropdownMenu.Item 
                              onClick={(e) => { e.stopPropagation(); setEditingFile(file.name); setEditNameValue(file.name); }}
                            ...
                            <DropdownMenu.Item 
                              onClick={(e) => { e.stopPropagation(); handleDuplicateFile(file.name); }}
                              className="flex items-center gap-2 px-3 py-2 text-sm text-on-surface hover:bg-surface-dim rounded-lg cursor-pointer outline-none"
                            >
                ...
                <div className="flex items-center gap-4">
                  ...
                  {activeFile && (
                    <div className="flex items-center gap-2 bg-[#1e1e1e] px-3 py-1 rounded-t-lg -mb-2 border-t border-l border-r border-[#404040]">
                      ...
                      <span className="text-[#cccccc] text-xs font-mono">{activeFile.name}</span>
                    </div>
                  )}
                </div>
                {activeFile && (
            ...
            <div className="w-full lg:w-[400px] xl:w-[500px] bg-surface-container-lowest rounded-3xl border border-surface-dim/20 shadow-2xl overflow-hidden flex flex-col shrink-0">
              <div className="bg-surface-container-low px-4 py-3 flex items-center justify-between border-b border-surface-dim/20">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-primary text-xs font-bold uppercase tracking-widest">Live Output & Preview</span>
                </div>
              </div>
              <div className="flex-1 min-h-0 relative bg-white">
                {showIframe ? (
                  ...
                  <div className="w-full h-full p-6 overflow-auto font-mono text-sm space-y-2 bg-[#0a0a0e] text-[#dce8ff]">
                    {output.length === 0 ? (
                      <span className="text-[#485468] italic">Click Run Code to execute scripts or preview HTML...</span>
                    ) : (
                      output.map((line, i) => (
                        <motion.div 
                        ...
                        >
                          <span className="text-[#485468] mr-4">{'>'}</span>{line}
                        </motion.div>
                      ))
                    )}
                  </div>
