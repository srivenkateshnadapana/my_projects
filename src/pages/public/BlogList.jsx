      ...
      <div className="max-w-7xl mx-auto">
        ...
        {blogs.length === 0 ? (
          <div className="text-center py-20 bg-surface-container-low rounded-[3rem] border border-surface-dim/20">
            ...
            <h3 className="text-xl font-bold text-primary mb-2">No blogs found</h3>
            <p className="text-on-surface-variant">Check back later for new content!</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {blogs.map(blog => (
              <div key={blog.id} className="bg-surface-container-low border border-surface-dim/20 rounded-[2rem] overflow-hidden flex flex-col group hover:-translate-y-2 hover:shadow-xl transition-all duration-300">
                {blog.imageUrl ? (
