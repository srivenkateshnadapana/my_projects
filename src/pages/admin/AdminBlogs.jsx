...
function AdminBlogsContent() {
  ...
  if (loading && !isEditing) {
    return <div className="min-h-screen flex items-center justify-center bg-surface"><Loader2 className="w-12 h-12 animate-spin text-primary" /></div>;
  }
        ...
        {isEditing ? (
          <div className="bg-surface-container-lowest border border-surface-dim/20 rounded-[3rem] p-8 shadow-xl animate-in fade-in zoom-in duration-200">
            <div className="flex justify-between items-center mb-8 border-b border-surface-dim/10 pb-6">
              <h2 className="text-2xl font-headline font-bold text-primary">{currentBlog.id ? 'Edit Blog' : 'Create Blog'}</h2>
              <button onClick={() => setIsEditing(false)} className="p-2 bg-surface-container rounded-full text-secondary hover:text-primary transition-colors">
                <X className="w-6 h-6" />
              </button>
            </div>
            ...
            <form onSubmit={handleSave} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  <label className="block text-xs font-bold text-secondary uppercase tracking-wider mb-2">Publishing Status</label>
                  <select 
              ...
              <div>
                <label className="block text-xs font-bold text-secondary uppercase tracking-wider mb-2">Content (Markdown/HTML supported) *</label>
                <textarea 
                  required
                  rows={12}
              ...
              <div className="flex justify-end gap-4 pt-6 border-t border-surface-dim/10">
                <button type="button" onClick={() => setIsEditing(false)} className="px-8 py-3 rounded-xl font-bold text-secondary hover:bg-surface-container transition-colors">
                  Cancel
                </button>
                <button type="submit" className="flex items-center gap-2 px-10 py-3 signature-gradient text-white rounded-xl font-bold shadow-lg hover:opacity-90 active:scale-95 transition-all">
                  <Save className="w-5 h-5" /> {currentBlog.id ? 'Save Changes' : 'Publish Blog'}
                </button>
              </div>
            </form>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {blogs.map(blog => (
              <div key={blog.id} className="bg-surface-container-lowest border border-surface-dim/10 rounded-[2rem] overflow-hidden flex flex-col hover:border-primary/30 hover:shadow-xl transition-all duration-300 group">
                <div className="h-48 bg-surface-container overflow-hidden">
                  <img 
                    src={blog.imageUrl || 'https://images.unsplash.com/photo-1499750310107-5fef28a66643?w=800&q=80'} 
                    alt={blog.title} 
                ...
                <div className="p-6 flex flex-col flex-grow">
                  <div className="flex items-center gap-2 mb-3">
                    <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest ${
                      blog.status === 'published' ? 'bg-success/10 text-success' : 'bg-warning/10 text-warning'
                    }`}>
                      {blog.status}
                    </span>
                    <span className="text-[10px] font-bold text-outline uppercase tracking-widest">{new Date(blog.createdAt).toLocaleDateString()}</span>
                  </div>
                  <h3 className="text-xl font-headline font-bold text-primary mb-4 line-clamp-2 leading-tight">{blog.title}</h3>
                  
                  <div className="mt-auto pt-6 border-t border-surface-dim/10 flex justify-end gap-2">
                    <button onClick={() => { setCurrentBlog(blog); setIsEditing(true); }} className="p-3 text-secondary hover:text-primary hover:bg-primary/5 rounded-2xl transition-all">
                      <Edit className="w-5 h-5" />
                    </button>
                    <button onClick={() => handleDelete(blog.id)} className="p-3 text-error/60 hover:text-error hover:bg-error/5 rounded-2xl transition-all">
            ...
            {blogs.length === 0 && (
              <div className="col-span-full text-center py-24 bg-surface-container-lowest rounded-[3rem] border border-dashed border-surface-dim/30">
                ...
                <h3 className="text-xl font-headline font-bold text-secondary">No blogs found</h3>
                <p className="text-outline text-sm mt-2">Initialize your thoughts by creating your first blog post.</p>
              </div>
            )}
          </div>
        )}
