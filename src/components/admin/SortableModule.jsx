    ...
    <div ref={setNodeRef} style={style} className="bg-surface-container-lowest border border-surface-dim/20 rounded-3xl overflow-hidden shadow-lg mb-6">
      <div className="bg-surface-container-low p-6 flex justify-between items-center border-b border-surface-dim/20 group">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => openModal('quiz', modQuiz, module.id, 'module')} className="p-2 text-success bg-success/10 rounded-lg hover:bg-success/20" title={modQuiz ? "Edit Module Quiz" : "Add Module Quiz"}><HelpCircle className="w-5 h-5" /></button>
          <button onClick={() => openModal('module', module)} className="p-2 text-secondary hover:text-primary"><Edit className="w-5 h-5" /></button>
          <button onClick={() => handleDelete('module', module.id)} className="p-2 text-error hover:bg-error/10 rounded-lg"><Trash2 className="w-5 h-5" /></button>
        </div>
      </div>
        ...
        {modQuiz && (
          <div className="flex justify-between items-center p-4 bg-surface-container-low rounded-2xl border border-surface-dim/20">
             <div className="flex items-center gap-4 pl-10">
                ...
                <div>
                  <p className="font-bold text-on-primary-container">{modQuiz.title}</p>
              ...
              <div className="flex gap-3 items-center">
                ...
                <button onClick={() => handleDelete('quiz', modQuiz.id)} className="text-error/60 hover:text-error"><Trash2 className="w-4 h-4" /></button>
              </div>
          </div>
        )}
      </div>
